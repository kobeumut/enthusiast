...
def index_all_documents_task(data_set_id: int):
    ...
    # Count in SQL and stream IDs via ``.iterator()`` so the backfill does not materialize every
    # document ID into memory just to log the queue size.
    documents = data_set.documents.all()
    logger.info("Queuing indexing for %s documents (data_set=%s)", documents.count(), data_set_id)
    for document_id in documents.values_list("id", flat=True).iterator():
...
def index_all_products_task(data_set_id: int):
    data_set = DataSet.objects.get(id=data_set_id)
    # Count in SQL and stream IDs via ``.iterator()`` so the backfill does not materialize every
    # product ID into memory just to log the queue size.
    products = data_set.products.all()
    logger.info("Queuing indexing for %s products (data_set=%s)", products.count(), data_set_id)
    for product_id in products.values_list("id", flat=True).iterator():
